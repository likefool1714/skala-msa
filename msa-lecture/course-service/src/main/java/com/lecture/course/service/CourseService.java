...
public class CourseService {
     ...
     */
    @Transactional
    public CourseDto.CourseResponse createCourse(CourseDto.CreateRequest request, Long carrierId) {
     ...
     * 카테고리별 강의 조회
     */
    public List<CourseDto.CourseResponse> getCoursesByCategory(Course.WasteType wasteType) {
        return courseRepository.findByWasteTypeAndStatus(wasteType, Course.Status.ACTIVE).stream()
                .map(CourseDto.CourseResponse::from)
                .collect(Collectors.toList());
    }
    ...
    public List<CourseDto.CourseResponse> getRecommendCourses(
            Course.WasteType wasteType, List<Long> excludeCourseIds) {

        List<Course> courses = excludeCourseIds.isEmpty()
                ? courseRepository.findByWasteTypeAndStatus(wasteType, Course.Status.ACTIVE)
                : courseRepository.findByWasteTypeAndStatusAndIdNotIn(
                        wasteType, Course.Status.ACTIVE, excludeCourseIds);

        // 수강생 수 기준 내림차순 정렬
        return courses.stream()
                .sorted((a, b) -> b.getRequestCount() - a.getRequestCount())
                .map(CourseDto.CourseResponse::from)
                .collect(Collectors.toList());
    }
    ...
    private Course findCourseById(Long id) {
        return courseRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("수거 서비스를 찾을 수 없습니다: " + id));
    }
}
