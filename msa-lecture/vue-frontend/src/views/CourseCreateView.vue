        ...
        <div class="form-card">
          <form class="course-form" @submit.prevent="handleSubmit">
            <div class="form-group">
              <label class="form-label" for="title">수거 서비스명</label>
              <input
                id="title"
                v-model.trim="form.name"
                type="text"
                class="form-input"
                placeholder="예: 서울권 일반의료폐기물 정기 수거"
                maxlength="100"
              />
            </div>

            <div class="form-group">
              <label class="form-label" for="description">서비스 설명</label>
              <textarea
                id="description"
                v-model.trim="form.description"
                class="form-textarea"
                rows="6"
                placeholder="수거 가능 지역, 운영 시간, 처리 가능 품목과 조건을 입력해 주세요."
              ></textarea>
            </div>

            <div class="form-row">
              <div class="form-group">
                <label class="form-label" for="category">폐기물·수거 유형</label>
                <select id="category" v-model="form.wasteType" class="form-select">
                  <option disabled value="">처리 가능 유형을 선택하세요</option>
                  <option
...
const categoryOptions = [
  { label: '일반의료폐기물', value: 'GENERAL_MEDICAL' },
...
function validateForm() {
  ...
  if (!auth.user || auth.user.role !== 'INSTRUCTOR') {
    validationError.value = '수집·운반 업체 계정만 수거 서비스를 등록할 수 있습니다.'
    return false
  }

  if (!form.name) {
    validationError.value = '수거 서비스명을 입력해 주세요.'
    return false
  }

  if (!form.description) {
    validationError.value = '수거 서비스 설명을 입력해 주세요.'
    return false
  }

  if (!form.wasteType) {
    validationError.value = '폐기물 유형을 선택해 주세요.'
    return false
  }
...
async function handleSubmit() {
  ...
  try {
    ...
    submitSuccess.value = '수거 서비스가 성공적으로 등록되었습니다.'
