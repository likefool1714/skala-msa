...
from app.model.schemas import WasteType, CollectionServiceResponse, RecommendResponse
...
class RecommendService:
    ...
    async def get_recommendations(self, user_id: int) -> RecommendResponse:
        ...
        active_course_ids = history.confirmedCollectionServiceIds
        ...
        return RecommendResponse(
            generatorId=user_id,
            recommendedCollectionServices=recommended,
            basedOnWasteType=dominant_category,
            message=f"{dominant_category.value} 유형 기반 추천 수거 서비스입니다"
        )
    ...
    async def _recommend_for_new_user(self, user_id: int) -> RecommendResponse:
        ...
        popular = sorted(
            all_courses,
            key=lambda c: c.requestCount,
            reverse=True
        )[:self.MAX_RECOMMEND_COUNT]

        return RecommendResponse(
            generatorId=user_id,
            recommendedCollectionServices=popular,
            basedOnWasteType=None,
            message="이용 건수가 많은 수거 서비스입니다"
        )
