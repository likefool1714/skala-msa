...
from app.model.schemas import CollectionHistoryResponse
...
class EnrollmentServiceClient:
    ...
    async def get_enrollment_history(self, user_id: int) -> CollectionHistoryResponse:
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                data = response.json()
                return CollectionHistoryResponse(**data)
        except httpx.HTTPError as e:
            logger.error(f"[EnrollmentClient] 수강 이력 조회 실패 - userId: {user_id}, error: {e}")
            # 실패 시 빈 이력 반환 (추천 서비스는 비핵심 기능)
            return CollectionHistoryResponse(generatorId=user_id, confirmedCollectionServiceIds=[])
