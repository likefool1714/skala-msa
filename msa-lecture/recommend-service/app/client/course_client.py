...
from app.model.schemas import CollectionServiceResponse, WasteType
...
class CourseServiceClient:
    ...
    async def get_recommend_courses(
    ...
    ) -> List[CollectionServiceResponse]:
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                return [CollectionServiceResponse(**c) for c in response.json()]
        except httpx.HTTPError as e:
            logger.error(f"[CourseClient] 추천 강의 조회 실패 - category: {category}, error: {e}")
            return []

    async def get_all_courses(self) -> List[CollectionServiceResponse]:
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                return [CollectionServiceResponse(**c) for c in courses]
        except httpx.HTTPError as e:
