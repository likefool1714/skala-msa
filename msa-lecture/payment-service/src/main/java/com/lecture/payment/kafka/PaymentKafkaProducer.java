...
public class PaymentKafkaProducer {
    ...
    public void publishPaymentCompleted(PaymentCompletedEvent event) {
        log.info("[Kafka Producer] payment.completed 발행 시도 - topic: {}, paymentId: {}, userId: {}, courseId: {}",
                paymentCompletedTopic, event.getPaymentId(), event.getGeneratorId(), event.getCollectionServiceId());

        try {
            SendResult<String, Object> result = kafkaTemplate
                    .send(paymentCompletedTopic, String.valueOf(event.getGeneratorId()), event)
